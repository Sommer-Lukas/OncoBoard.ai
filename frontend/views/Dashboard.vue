...
const phases = [
  {
    ...
    status: 'complete',
    statusLabel: 'Ready',
    description: '7 agents completed · 1 data gap flagged',
  },
  {
    key: 'meeting',
    label: 'Meeting Room',
    ...
    description: 'Board convened · Transcript recording',
  },
  {
    key: 'post',
    label: 'Post-Meeting',
    ...
    status: 'pending',
    statusLabel: 'Pending',
    description: 'Awaiting board decisions',
  },
]
...
<template>
  <div class="dashboard">
    <div class="container">
      ...
      <!-- Page header — same style as PostMeeting -->
      ...
      <div class="phase-row">
        <template v-for="(phase, idx) in phases" :key="phase.key">
          <div
            class="phase-card"
            :class="phase.status"
            @click="router.push(phase.route)"
          >
            <div class="phase-icon-box" :style="{ background: statusConfig[phase.status].bg }">
              <span class="material-symbols-outlined" :style="{ fontSize: '20px', color: statusConfig[phase.status].dot }">
                {{ phase.icon }}
              </span>
            </div>
            ...
            <div
              class="phase-chip"
              :style="{ background: statusConfig[phase.status].bg, color: statusConfig[phase.status].color }"
            >
              {{ phase.statusLabel }}
            </div>
          </div>
      ...
      <!-- Active case -->
      <div class="section-label" style="margin-top: 32px">Active Case</div>
      <div class="case-card">
        <div class="case-top">
          <div class="case-avatar">
            <span class="material-symbols-outlined" style="font-size: 22px; color: #1A73E8">person</span>
          </div>
          <div class="case-meta">
        ...
        <div class="case-actions">
          <button class="action-btn" @click="router.push('/pre-meeting')">
            <span class="material-symbols-outlined" style="font-size: 16px">checklist</span>
            Pre-Meeting
          </button>
          <button class="action-btn" @click="router.push('/post-meeting')">
